...
const PARAM_LABELS: Record<string, { title: string; unit: string; color: string }> = {
  hr_mean: { title: "Puls (Tinch holatda)", unit: "bpm", color: "#B3261E" },
      ...
      <div className="chart-card-header">
        <div>
          ...
          {series.baseline_median !== null && (
            <span className="chart-meta" style={{ marginLeft: "12px" }}>
              Baza: {series.baseline_median} {meta.unit} (norma: {series.baseline_low} - {series.baseline_high})
            </span>
          )}
        </div>
        {hasDeviations && (
          <span className="deviations-tag" style={{ fontSize: "11px" }}>
            Og'ish qayd etilgan
          </span>
        )}
      </div>
